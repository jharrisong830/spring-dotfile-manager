...
- **`bin/`**: `build.sh`, `install.sh`, `sdfm` — shell scripts for building/installing outside of Maven directly, and the thin wrapper script that invokes the installed jar. `build.ps1`, `install.ps1`, `sdfm.cmd` are the Windows (PowerShell) equivalents — same structure, installing to `%LOCALAPPDATA%\spring-dotfile-manager\bin` instead of `~/.local/bin`.
...
- Platform-dispatched, like `ConfigServiceImpl`'s `configFilePath`: `PostInstallServiceImpl`'s constructor takes three `@Value`-injected glob patterns (`post-install-glob-pattern.linux`/`.darwin`/`.win32` in `application.yaml`) and resolves the one for the current platform (`os.name`, via `FormattingUtils.getResolvedOsName`) into a single field via `switch`. Linux/macOS default to `post-install/**.sh`; Windows defaults to `post-install/**.ps1` — so which scripts get picked up differs by platform, not just how they're run. Note the pattern is `**.sh`, not `**/*.sh`: Java NIO glob syntax requires a literal `/` between `**` and a following `*`, so `**/*.sh` only matches files in a *subdirectory* of `post-install/` and silently misses scripts placed directly inside it — `**.sh` matches both (see `FileServiceUnitTests#testGlob_doubleStarSlashPattern_doesNotMatchDirectChildOfLiteralPrefix` vs `#testGlob_doubleStarPattern_matchesDirectAndNestedChildOfLiteralPrefix`).
- Scripts are discovered recursively under `post-install/` at the root of the dotfile repo, matching the resolved glob pattern.
...
- Each script is run via a platform-dispatched command (`switch` on `osName` in `runPostInstallScripts()`): `bash <script>` on Linux/macOS, `pwsh -NoProfile -NonInteractive -ExecutionPolicy Bypass -File <script>` on Windows — with `cwd` set to the script's own parent directory. The Windows path requires PowerShell 7+ (`pwsh`) on `PATH`; the older, preinstalled Windows PowerShell (`powershell`) is not used, and there's no fallback if `pwsh` is missing (surfaces as a per-script failure, e.g. `IOException: Cannot run program "pwsh"`).
